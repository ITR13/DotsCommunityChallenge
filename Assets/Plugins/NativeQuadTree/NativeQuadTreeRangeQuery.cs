    ...
    public unsafe partial struct NativeQuadTree<T> where T : unmanaged
    {
        struct QuadTreeRangeQuery
        {
            ...
            public void Query(NativeQuadTree<T> tree, AABB2D bounds, NativeList<QuadElement<T>> results)
            {
                ...
                RecursiveRangeQuery(tree.bounds[0], false, 1, 1);
            ...
            public void RecursiveRangeQuery(AABB2D parentBounds, bool parentContained, int prevOffset, int depth)
            {
                ...
                for (int l = 0; l < 4; l++)
                {
                    ...
                    if (!contained)
                    {
                        ...
                        {
                            // Debug.Log($"{parentBounds} - {childBounds} ({l}) -> Does not intersect");
                            continue;
                        }
                    }
                    ...
                    {
                        // Debug.Log($"{parentBounds} - {childBounds} ({l}) -> Recursive");
                        RecursiveRangeQuery(childBounds, contained, at + 1, depth + 1);
                    }
                    else if (elementCount != 0)
                    {
                        // Debug.Log($"{parentBounds} - {childBounds} ({l}) -> {elementCount} ({contained})");
                        ...
                        if (contained)
                        {
                            ...
                            UnsafeUtility.MemCpy(
                                (void*)((IntPtr)fastResults->Ptr + count * UnsafeUtility.SizeOf<QuadElement<T>>()),
                                index,
                    ...
                    {
                        // Debug.Log($"{parentBounds} - {childBounds} ({l}) -> Is empty");
                    }
                }
