...
namespace NativeQuadTree
{
    ...
    public unsafe partial struct NativeQuadTree<T> : IDisposable where T : unmanaged
    {
        ...
        NativeArray<AABB2D> bounds; // NOTE: Currently assuming uniform
        ...
        {
            this.bounds = CollectionHelper.CreateNativeArray<AABB2D>(1, allocator);
...
#if ENABLE_UNITY_COLLECTIONS_CHECKS && DISPOSE_SENTINEL && DISPOSE_SENTINEL
        ...
        public void ClearAndBulkInsert(in NativeArray<QuadElement<T>> incomingElements, in AABB2D bounds)
        {
            ...
            var depthExtentsScaling = LookupTables.DepthLookup[maxDepth] / this.bounds[0].Extents;
            for (var i = 0; i < incomingElements.Length; i++)
            {
                var incPos = incomingElements[i].pos;
                incPos -= this.bounds[0].Center; // Offset by center
                incPos.y = -incPos.y; // World -> array
                var pos = (incPos + this.bounds[0].Extents) * .5f; // Make positive
